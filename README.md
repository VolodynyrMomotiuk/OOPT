...
-	Адаптер (Adapter).
-	
Технічне завдання до цього коду полягає в створенні адаптера для забезпечення сумісності між класами OnlineCourse і ICourse.
> - Інтерфейс ICourse визначає спільний набір методів, які використовуються для доступу до курсів. Цей інтерфейс має три методи: Start(), SubmitAssignment(string assignment) і CommunicateWithInstructor(string message). Ці методи дозволяють починати курс, надсилати завдання для перевірки і спілкуватися з викладачем відповідно.
> - Клас OnlineCourse є адаптованим класом, який вже існує у системі і надає методи LaunchCourse(), SubmitTask(string task) і ContactInstructor(string message). Проте цей клас не реалізує інтерфейс ICourse, і ми хочемо забезпечити можливість використовувати його через цей інтерфейс.
> - Клас OnlineCourseAdapter є адаптером, який реалізує інтерфейс ICourse. У конструкторі адаптера приймається об'єкт типу OnlineCourse, який потрібно адаптувати. Методи Start(), SubmitAssignment(string assignment) і CommunicateWithInstructor(string message) в адаптері викликають відповідні методи в об'єкті OnlineCourse, забезпечуючи таким чином перетворення інтерфейсу.
